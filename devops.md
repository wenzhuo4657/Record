...
##### 结尾
  ...
  ​	有点忙，后端代码没有重构完成，但算是基本整理了一个新版本，然后通知对接还没有做，打算后续改成微服务的，先不写

#### 第二阶段：ui重构（12.27-1.2）
给前端提供菜单类型，将文档类型作为菜单列表的一部分，而文档列表作为菜单内容。
...
1，便于新增文档： 使用左侧菜单栏展示文档类型，右侧先展示文档里列表

2，入口日报： 仍然需要将日报编写作为首位展示，可以尝试使用 格式为 #文档类型：文档名称 来表示编写的内容归属的文档

3，入口页面：   入口日报+  预览内容（暂无想法）
